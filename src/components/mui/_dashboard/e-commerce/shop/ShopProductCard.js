...
import { MIconButton } from 'src/allTemplateComponents/@material-extend';
import { Icon } from '@iconify/react';
import closeFill from '@iconify/icons-eva/close-fill';
import { deleteItemFromFavoriteList, saveItemToFavoriteList } from 'src/utils/localstorage';
...
export default function ShopProductCard({
  ...
  const errorFavoritingSnackbar = () => {
    closeSnackbar();
    enqueueSnackbar(`Something Went wrong, your Favorite Item is reverted, please try again later to favorite`, {
      variant: 'error',
      action: (key) => (
        <MIconButton size="small" onClick={() => closeSnackbar(key)}>
          <Icon icon={closeFill} />
  ...
  const unFavoriteVehicle = () => {
    axios.delete(`${process.env.NEXT_PUBLIC_STRAPI_URL}/favorites/${product.favoriteId}`, {
    ...
    <Card>
      <Box width="10%" mx="1rem" my="0.5rem">
        {isLoading ? (
          ...
          <img
            src={`/static/icons/${product.isFavourite ? 'heart_filled.svg' : 'heart_empty.svg'
              }`}
        ...
        >
          ...
          <Link href={linkTo} color="inherit" component={RouterLink}>
            <Typography variant="subtitle1">
              ...
              >
                {intPrice === 0 ? 'Price available on request' : fCurrency(intPrice)}
              </Typography>
            </Typography>
          </Link>
        </Stack>
