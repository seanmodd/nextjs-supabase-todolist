...
const ALLCARSQUERY = (dataPerPage, pageNum, filters) => gql`
  query Variants {
    variants(
      ...
      where: {
        ${filters.make?.length ? `make_in: ${JSON.stringify(filters.make)},` : ''}
        ${filters.dealership?.length ? `car_dealership_in: ${JSON.stringify(filters.dealership)},` : ''}
...
// ----------------------------------------------------------------------
export function getVariants(dataPerPage = 24, pageNum = 1, filters = {
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      console.log('product.js	491	response.data.variants', print(q), response.data.variants);
